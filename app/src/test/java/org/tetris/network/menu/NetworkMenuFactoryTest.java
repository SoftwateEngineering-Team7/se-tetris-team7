    ...
    public void testCreateBundle() {
        ...
        try {
            ...
            if (bundle != null) {
                ...
                // Model 검증
                assertTrue("Model은 NetworkMenu 타입이어야 합니다", 
                          bundle.model() instanceof NetworkMenu);
                NetworkMenu networkMenu = (NetworkMenu) bundle.model();
                assertNotNull("Model의 IP 주소가 null이 아니어야 합니다", 
                             networkMenu.getIpAddress());
                assertTrue("Model의 포트는 0보다 커야 합니다", 
                          networkMenu.getPort() > 0);
            ...
            // FXML 파일이 없거나 JavaFX Toolkit 초기화 실패한 경우
            String message = e.getMessage();
            assertTrue("FXML 로딩 실패 또는 JavaFX Toolkit 오류가 예상됩니다", 
                      message != null && (message.contains("Failed to load FXML") || 
                                         message.contains("Toolkit not initialized") || 
                                         e.getCause() instanceof ExceptionInInitializerError));
            System.out.println("예상된 오류 (JavaFX 환경 필요): " + e.getMessage());
        }
    }
